...
<head>
    ...
    <link rel="stylesheet" href="${pageContext.request.contextPath}/css/page_index_detailActivity.css" type="text/css">
    ...
    <script>
        $(function (){
            ...
            $("#submitEnroll").click(function (){
                var loginUser = "<%=session.getAttribute("sid")%>";
                if(date.isDuringDate(enrollEndTime, '2050-09-17 15:00')){
                    alert("活动已结束！");
                }
...
<div class="content w clearfix">
    ...
    <div class="detail">
        ...
        <span>开始时间：${activity.actBeginTime}</span>
        <span>结束时间：${activity.actEndTime}</span>
        ...
        <div style="overflow: auto; ">${activity.actContent}</div>
        <form id="submitForm" action="enroll">
            <input readonly="readonly" value="${activity.id}" name="actId" style="display: none">
            <input readonly="readonly" value="${activity.category}" name="category" style="display: none">
            <input readonly="readonly" value="${activity.managerId.id}" name="managerId" style="display: none">
            <input readonly="readonly" value="${activity.tuanId._class}" name="_class" style="display: none">
            <button type="button" id="submitEnroll" class="btn btn-primary">报名</button>
        </form>
    </div>
</div>
<!--内容结束-->
